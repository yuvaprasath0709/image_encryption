...
def encrypt_image(path):
    try:
        ...
        print(colorize(f"[✔] Encrypted → {output_path}", "green"))
...
def interactive_mode():
    ...
    action = input("Do you want to (e)ncrypt or (d)ecrypt? (or 'q' to quit): ").strip().lower()
    if action in ['q', 'quit']:
        print(colorize("\n Exiting image encryption tool. Stay safe!\n", "green"))
        return
...
def print_usage():
    print("\nUsage:")
    print("  python image_encryptor.py encrypt <image_path>")
    print("  python image_encryptor.py decrypt <image_path.enc>")
...
def main():
    ...
    action, file_path = sys.argv[1].lower(), sys.argv[2]
